...
export const signTokens = async (user: DocumentType<User>) => {
  ...
  const accessToken = signJwt({ user: user_id }, "accessPrivateKey", {
    expiresIn: `${ACCESS_TOKEN_EXPIRES_IN}m`,
  });

  const refreshToken = signJwt({ user: user_id }, "refreshPrivateKey", {
...
export const reIssueNewAccessToken = async (refreshToken: string) => {
  ...
  return signJwt({ user: String(user._id) }, "accessPrivateKey", {
    expiresIn: `${ACCESS_TOKEN_EXPIRES_IN}m`,
  });
};
